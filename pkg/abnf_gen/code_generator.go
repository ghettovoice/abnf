...
type CodeGenerator struct {
	...
	PackageName string
	// Wrap or not errors with errtrace package.
	WrapErrors bool
...
const (
	mainPkg     = "github.com/ghettovoice/abnf"
	errtracePkg = "braces.dev/errtrace"
)
...
func (g *CodeGenerator) WriteTo(dst io.Writer) (int64, error) {
	if g.code.Len() == 0 {
		...
		f.ImportName("sync", "")
		if g.WrapErrors {
			f.ImportName(errtracePkg, "errtrace")
		}
			...
			opStmt := jen.
				...
				Block(
					...
					jen.ReturnFunc(func(gr *jen.Group) {
						stmt := jen.Id("desc").Dot(r.privName()).Call(
							jen.Id("in"),
							jen.Id("pos"),
							jen.Id("ns"),
						)
